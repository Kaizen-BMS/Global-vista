  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-5">
        ...
        <form onSubmit={addCountry} className="flex gap-2 mb-3"><input value={newCountry} onChange={(e) => setNewCountry(e.target.value)} className="flex-1 px-3 py-2 rounded-lg bg-neutral-800 border border-neutral-700 text-white text-sm" /><button type="submit" className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm"><Plus className="h-4 w-4" /></button></form>
        <div className="space-y-1">{countries.map((c) => <button key={c.id} onClick={() => setSelectedCountryId(c.id)} className={`w-full text-left px-3 py-2 rounded-lg text-sm ${selectedCountryId === c.id ? "bg-indigo-600/10 text-indigo-400" : "text-neutral-300 hover:bg-neutral-800"}`}>{c.name}</button>)}</div>
      </div>
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-5">
        <h3 className="text-white font-medium mb-3">States</h3>
        <form onSubmit={addState} className="flex gap-2 mb-3"><input value={newState} onChange={(e) => setNewState(e.target.value)} disabled={!selectedCountryId} className="flex-1 px-3 py-2 rounded-lg bg-neutral-800 border border-neutral-700 text-white text-sm" /><button type="submit" disabled={!selectedCountryId} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm"><Plus className="h-4 w-4" /></button></form>
        <div className="space-y-1">{states.map((s) => <button key={s.id} onClick={() => setSelectedStateId(s.id)} className={`w-full text-left px-3 py-2 rounded-lg text-sm ${selectedStateId === s.id ? "bg-indigo-600/10 text-indigo-400" : "text-neutral-300 hover:bg-neutral-800"}`}>{s.name}</button>)}</div>
      </div>
      <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-5">
        <h3 className="text-white font-medium mb-3">Cities</h3>
        <form onSubmit={addCity} className="flex gap-2 mb-3"><input value={newCity} onChange={(e) => setNewCity(e.target.value)} disabled={!selectedStateId} className="flex-1 px-3 py-2 rounded-lg bg-neutral-800 border border-neutral-700 text-white text-sm" /><button type="submit" disabled={!selectedStateId} className="px-3 py-2 rounded-lg bg-indigo-600 text-white text-sm"><Plus className="h-4 w-4" /></button></form>
