  ...
  return (
    <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 max-w-2xl">
      ...
      <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-4 gap-2 mb-6">
        ...
        <label className="flex items-center gap-2 text-sm text-neutral-300 sm:col-span-3"><input type="checkbox" checked={form.isCurrent} onChange={(e) => setForm({ ...form, isCurrent: e.target.checked })} />Current session</label>
        <button type="submit" disabled={saving} className="flex items-center justify-center gap-1 px-3 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm disabled:opacity-60">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />}</button>
      </form>
      <div className="divide-y divide-neutral-800">{sessions.map((s) => <div key={s.id} className="flex items-center justify-between py-2"><div><span className="text-neutral-300 text-sm">{s.name}</span>{!!s.is_current && <CheckCircle2 className="inline h-3.5 w-3.5 text-green-400 ml-2" />}</div><button onClick={() => handleDelete(s.id)}><Trash2 className="h-4 w-4 text-neutral-500 hover:text-red-400" /></button></div>)}</div>
    </div>
  );
}