...
export default function AcademicSessionsEditor({ sessions }) {
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-6 max-w-2xl">
      <h2 className="text-foreground font-medium mb-4">Academic Sessions</h2>
      <form onSubmit={handleAdd} className="grid grid-cols-1 sm:grid-cols-4 gap-2 mb-6">
        <input placeholder="Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="sm:col-span-2 px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm" required />
