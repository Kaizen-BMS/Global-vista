...
export default function ReportToolbar({ exportBase, title }) {
  return (
    <div className="flex items-center gap-2 print:hidden">
      <a href={`${exportBase}?format=xlsx`} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-800 text-neutral-300 hover:text-white text-sm transition">
        <Download className="h-4 w-4" /> Excel
      </a>
      <a href={`${exportBase}?format=csv`} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-neutral-900 border border-neutral-800 text-neutral-300 hover:text-white text-sm transition">
