...
export default function ResetPasswordClient() {
  ...
  const [tokenState, setTokenState] = useState(null); // 'valid' | 'invalid' | 'expired' | 'missing'
  ...
  useEffect(() => {
    ...
    fetch(`/api/auth/reset-password?token=${encodeURIComponent(token)}`)
      .then((res) => res.json())
      .then((data) => {
        setTokenState(data.valid ? "valid" : data.reason || "invalid");
        setChecking(false);
      })
      .catch(() => {
  ...
  async function handleSubmit(e) {
    ...
    if (strength < 4) {
      toast.error("Use a stronger password (uppercase, lowercase, number, special character).");
      return;
    }
    setSaving(true);
    try {
      ...
      >
        ...
        {!checking && success && (
          <div className="text-center py-6">
            ...
            <p className="text-neutral-500 text-sm">Redirecting you to the dashboard…</p>
          </div>
        )}
        ...
        {!checking && !success && tokenState === "expired" && (
          <div className="text-center py-6">
            ...
            <p className="text-neutral-500 text-sm mb-6">This reset link has expired. Request a new one.</p>
            <a href="/crm/forgot-password" className="inline-block px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition">
              Request New Link
            </a>
          </div>
        )}
        ...
        {!checking && !success && (tokenState === "invalid" || tokenState === "missing") && (
          <div className="text-center py-6">
            ...
            <p className="text-neutral-500 text-sm mb-6">This reset link is invalid or has already been used.</p>
            <a href="/crm/forgot-password" className="inline-block px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition">
              Request New Link
            </a>
          </div>
        ...
        {!checking && !success && tokenState === "valid" && (
          <form onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={saving}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition disabled:opacity-60"
            >
