...
export default function NotificationBell() {
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((o) => !o)} className="relative text-muted-foreground hover:text-foreground cursor-pointer transition-colors" aria-label="Notifications">
      ...
      {open && <NotificationDrawer notifications={notifications} onClose={() => setOpen(false)} onRead={() => load({ silent: true })} />}
    </div>
  );
}
