...
import { Plus, Pencil, Trash2, GripVertical, FileText, AlertTriangle } from "lucide-react";
import { apiFetch } from "@/components/shared/apiClient";
import LeadDocumentTypeForm from "@/components/forms/LeadDocumentTypeForm";
import EmptyState from "@/components/shared/EmptyState";

export default function LeadDocumentTypeManager({ initialTypes, schemaReady }) {
  ...
  function refresh() { router.refresh(); setEditing(null); setCreating(false); }

  async function remove(type) {
    if (!confirm(`Delete "${type.name}"? It will no longer be offered when uploading lead documents. Documents already uploaded under it keep their history.`)) return;
    setBusyId(type.id);
    try {
      const res = await apiFetch(`/api/core/lead-document-types/${type.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-muted-foreground text-sm">Define the document categories your team collects from leads — fully custom, no code changes needed.</p>
      ...
      {types.length === 0 ? (
        <EmptyState icon={FileText} title="No document types yet" description="Add your first document type, e.g. Passport or Offer Letter." />
      ) : (
      ...
      {(creating || editing) && <LeadDocumentTypeForm initial={editing} onClose={() => { setCreating(false); setEditing(null); }} onSaved={refresh} />}
    </div>
  );
}
