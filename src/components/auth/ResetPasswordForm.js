...
function ResetFormInner({ onNavigate }) {
    ...
    return (
      <div className="text-center py-8">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium transition-all cursor-pointer hover:-translate-y-0.5"
        >
          Request New Link
        </button>
      </div>
...
export default function ResetPasswordForm({ onNavigate }) {
  return (
    <Suspense fallback={<div className="flex justify-center py-16"><Loader2 className="h-6 w-6 text-white/40 animate-spin" /></div>}>
      <ResetFormInner onNavigate={onNavigate} />
    </Suspense>
  );
}
