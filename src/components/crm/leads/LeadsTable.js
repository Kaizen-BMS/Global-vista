  ...
  return (
    <div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-neutral-500 border-b border-neutral-800">
              <th className="px-4 py-3 w-8"><input type="checkbox" checked={selected.length === leads.length} onChange={toggleAll} /></th>
              <th className="px-4 py-3">Lead #</th><th className="px-4 py-3">Name</th><th className="px-4 py-3">Phone</th>
              <th className="px-4 py-3">Country</th><th className="px-4 py-3">Source</th><th className="px-4 py-3">Assigned</th>
              <th className="px-4 py-3">Stage</th><th className="px-4 py-3">Priority</th>
            </tr>
          </thead>
          <tbody>
            {leads.map((lead) => (
              <tr key={lead.id} className="border-b border-neutral-800/60 hover:bg-neutral-800/30 transition">
                <td className="px-4 py-3"><input type="checkbox" checked={selected.includes(lead.id)} onChange={() => toggleSelect(lead.id)} /></td>
                <td className="px-4 py-3 text-neutral-400 text-xs">{lead.lead_number}</td>
                <td className="px-4 py-3">
                  <Link href={`/workspace/lead-management/${lead.id}`} className="text-white hover:text-indigo-400 font-medium">{lead.name}</Link>
      ...
      <div className="md:hidden space-y-3">
        {leads.map((lead) => (
          <Link key={lead.id} href={`/workspace/lead-management/${lead.id}`} className="block bg-neutral-900 border border-neutral-800 rounded-xl p-4">
