      ...
      {VIEWS.map((v) => {
        ...
        return (
          <Link
            key={v.key}
            href={v.href}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-sm transition cursor-pointer ${isActive ? "bg-indigo-600 text-white" : "text-neutral-400 hover:text-white hover:bg-neutral-800"}`}
          >
