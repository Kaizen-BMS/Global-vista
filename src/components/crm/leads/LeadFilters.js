...
export default function LeadFilters({ sources = [], services = [], counsellors = [], tags = [] }) {
  ...
  return (
    <div className="mb-4">
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-lg bg-card border border-border text-foreground text-sm transition focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
        </div>

        <select defaultValue={searchParams.get("stage") || ""} onChange={(e) => setParam("stage", e.target.value)} className="px-3 py-2 rounded-lg bg-card border border-border text-foreground text-sm cursor-pointer">
          <option value="">All Stages</option>
          {[...LEAD_STAGES, ...LEAD_TERMINAL_STAGES].map((s) => <option key={s} value={s}>{s}</option>)}
        </select>

        <select defaultValue={searchParams.get("priority") || ""} onChange={(e) => setParam("priority", e.target.value)} className="px-3 py-2 rounded-lg bg-card border border-border text-foreground text-sm cursor-pointer">
          <option value="">All Priorities</option>
          {LEAD_PRIORITIES.map((p) => <option key={p} value={p}>{p}</option>)}
        </select>

        <select defaultValue={searchParams.get("sourceId") || ""} onChange={(e) => setParam("sourceId", e.target.value)} className="px-3 py-2 rounded-lg bg-card border border-border text-foreground text-sm cursor-pointer">
          <option value="">All Sources</option>
          {sources.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
        </select>

        <select defaultValue={searchParams.get("serviceId") || ""} onChange={(e) => setParam("serviceId", e.target.value)} className="px-3 py-2 rounded-lg bg-card border border-border text-foreground text-sm cursor-pointer">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 px-3 py-2 rounded-lg border text-sm transition cursor-pointer ${advancedOpen || activeAdvancedCount > 0 ? "bg-indigo-500/10 border-indigo-500/40 text-indigo-300" : "bg-card border-border text-foreground hover:text-foreground"}`}
        >
          <SlidersHorizontal className="h-4 w-4" /> Advanced {activeAdvancedCount > 0 && `(${activeAdvancedCount})`}
        </button>

        <div className="relative">
          <button type="button" onClick={() => setSavedOpen((o) => !o)} className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-card border border-border text-foreground hover:text-foreground text-sm transition cursor-pointer">
            <Bookmark className="h-4 w-4" /> Saved
          </button>
          {savedOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-card border border-border rounded-xl shadow-2xl z-40 p-2 animate-in fade-in slide-in-from-top-1 duration-150">
              {savedFilters.length === 0 && <p className="text-muted-foreground text-xs text-center py-4">No saved filters yet.</p>}
              {savedFilters.map((f) => (
      ...
      {advancedOpen && (
        <div className="mt-2 p-4 bg-card border border-border rounded-xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 animate-in fade-in slide-in-from-top-1 duration-150">
          <div>
          ...
          <div>
            ...
            <input defaultValue={searchParams.get("country") || ""} onChange={(e) => setParam("country", e.target.value)} placeholder="e.g. Canada" className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm" />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Tag</label>
            <select defaultValue={searchParams.get("tag") || ""} onChange={(e) => setParam("tag", e.target.value)} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm cursor-pointer">
          ...
          <div>
            ...
            <input type="date" defaultValue={searchParams.get("createdFrom") || ""} onChange={(e) => setParam("createdFrom", e.target.value)} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm" />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Created To</label>
            <input type="date" defaultValue={searchParams.get("createdTo") || ""} onChange={(e) => setParam("createdTo", e.target.value)} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm" />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Follow-up From</label>
            <input type="date" defaultValue={searchParams.get("followupFrom") || ""} onChange={(e) => setParam("followupFrom", e.target.value)} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm" />
          </div>
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Follow-up To</label>
            <input type="date" defaultValue={searchParams.get("followupTo") || ""} onChange={(e) => setParam("followupTo", e.target.value)} className="w-full px-3 py-2 rounded-lg bg-muted border border-border text-foreground text-sm" />
          </div>
          <div className="col-span-full flex justify-end">
            <button type="button" onClick={clearAdvanced} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-muted-foreground hover:text-foreground text-xs transition cursor-pointer">
