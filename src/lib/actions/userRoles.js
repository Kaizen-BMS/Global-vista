...
// on its own — it must show up in user_roles for THIS session's own user
// id before the session token is re-signed (done by the caller via
// auth.js reissueSessionWithRole once this resolves).
export async function switchActiveRole(session, roleId) {
  const [[assignment]] = await pool.query(
    `SELECT ur.role_id, r.slug, r.name FROM user_roles ur JOIN roles r ON r.id = ur.role_id
     WHERE ur.user_id = ? AND ur.role_id = ? AND r.is_deleted = 0 LIMIT 1`,
    [session.id, roleId]
  );
