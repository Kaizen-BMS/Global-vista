...
export async function recordFailedLogin(userId, companyId) {
  ...
  if (next >= MAX_FAILED_ATTEMPTS) {
    ...
    await logActivity({ userId, module: "auth", action: "account_locked", entityType: "user", entityId: userId, description: `Locked after ${next} failed attempts`, companyId });
  } else {
