...
export async function terminateSession(sessionRowId, userId) {
  await pool.query(
...
export async function logoutAllDevices(userId, currentJti, actorId) {
  ...
  await logActivity({
    userId: actorId, module: "auth", action: "logout_all_devices",
    entityType: "user", entityId: userId, description: "Logged out of all other devices",
  });
}