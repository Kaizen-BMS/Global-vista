...
function getSecret() {
  return new TextEncoder().encode(process.env.CRM_JWT_SECRET);
}
...
export async function createSession(user, { ipAddress = null, userAgent = null, rememberMe = false } = {}) {
  ...
  await pool.query(
    `INSERT INTO user_sessions (user_id, jti, ip_address, user_agent) VALUES (?, ?, ?, ?)`,
  ...
  // CSRF token issued alongside every new session so it's available
...
export async function destroySession() {
  ...
  if (token) {
    try {
      ...
      if (payload.jti) {
        await pool.query(`UPDATE user_sessions SET revoked_at = NOW() WHERE jti = ?`, [payload.jti]);
      }
    } catch {
...
export async function destroyAllSessions(userId, exceptJti = null) {
  await pool.query(
    `UPDATE user_sessions SET revoked_at = NOW() WHERE user_id = ? AND revoked_at IS NULL ${exceptJti ? "AND jti != ?" : ""}`,
