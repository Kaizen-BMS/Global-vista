...
export default async function EmployeeProfilePage({ params }) {
  ...
  if (!user) return <div className="text-neutral-500 text-sm">Not found.</div>;
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div><h1 className="text-xl font-semibold text-white">{user.name}</h1><p className="text-neutral-500 text-sm">{user.role_name} · {user.email}</p></div>
      </div>
      <EmployeeProfileTabs user={user} loginHistory={loginHistory} activity={activity} />
    </div>
  );
}