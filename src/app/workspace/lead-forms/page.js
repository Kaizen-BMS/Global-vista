...
const STATUS_STYLES = { active: "bg-emerald-500/10 text-emerald-400 border-emerald-500/30", inactive: "bg-neutral-700/20 text-neutral-400 border-neutral-600/30" };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-white">Lead Forms</h1>
          <p className="text-neutral-500 text-sm mt-0.5">Public forms that turn into leads automatically — share the link, print the QR code, or embed it anywhere.</p>
        </div>
        {canCreate && (
      ...
      {forms.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {forms.map((f) => (
            <Link key={f.id} href={`/workspace/lead-forms/${f.id}`} className="block bg-neutral-900 border border-neutral-800 rounded-xl p-5 hover:border-neutral-700 hover:-translate-y-0.5 transition cursor-pointer">
              <div className="flex items-start justify-between mb-3">
                <p className="text-white font-medium truncate pr-2">{f.name}</p>
                <span className={`shrink-0 text-[10px] px-2 py-0.5 rounded-md border ${STATUS_STYLES[f.status]}`}>{f.status}</span>
              </div>
              <p className="text-neutral-500 text-xs truncate mb-4">/forms/{f.slug}</p>
