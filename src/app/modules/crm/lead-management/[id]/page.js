...
export default async function LeadDetailsPage({ params }) {
  ...
  const [notes, followups, tasks, documents, timeline, canEdit, canManageNotes, canManageFollowups, canManageTasks, canManageDocs] = await Promise.all([
    listLeadNotes(id, session),
    listLeadFollowups(id),
    listLeadTasks(id),
    listLeadDocuments(id),
    getLeadTimeline(id),
    can(session, "leads.update"),
    can(session, "leads.notes.manage"),
    can(session, "leads.followups.manage"),
    can(session, "leads.tasks.manage"),
    can(session, "leads.documents.manage"),
  ]);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        {canEdit && (
          <Link
            href={`/crm/lead-management/${id}/edit`}
