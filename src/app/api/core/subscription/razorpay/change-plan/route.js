...
export const POST = withCsrf(withErrorHandling(async (request) => {
  ...
  const { planId, when, couponCode, seatQuantity, durationMonths } = await request.json();
  ...
  const result = await changeCompanyRazorpayPlan(session, planId, when === "cycle_end" ? "cycle_end" : "now", couponCode?.trim() || null, seatQuantity, durationMonths || 1);
  return ok(result);
}));
