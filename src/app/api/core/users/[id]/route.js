...
export const PUT = withCsrf(withErrorHandling(async (request, ctx) => {
  ...
  if (body.action === "restore") { assertSuperAdmin(session); await restoreUser(session, id, session.id); return ok(); }
  ...
  return ok({ user: await updateUser(session, id, body, session.id) });
}));
