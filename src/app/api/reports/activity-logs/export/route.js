...
const COLUMNS = [
  ...
  ["user_name", "User"], ["ip_address", "IP Address"], ["created_at", "Date"],
];
...
export const GET = withErrorHandling(async (request) => {
  ...
  const logs = await getActivityLogs({ companyId: session.company_id, limit: 5000 });
  return buildExportResponse(logs, COLUMNS, { format, filenameBase: "activity-logs-export", sheetName: "Activity Logs" });
});
