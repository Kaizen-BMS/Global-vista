...
export const GET = withErrorHandling(async (request, context) => {
  ...
  const assignedUserCount = await getRoleAssignedUserCount(session, id);
...
export const PUT = withCsrf(withErrorHandling(async (request, context) => {
  ...
  await updateRole(session, id, body, session.id);
  return ok();
}));
...
export const DELETE = withCsrf(withErrorHandling(async (request, context) => {
  ...
  try {
    await deleteRole(session, id, session.id, reassignToRoleId);
    return ok();
  } catch (err) {
    if (err.requiresReassignment) return badRequest(err.message, { requiresReassignment: true, assignedCount: err.assignedCount });
    throw err;
  }
}));